...
export function DeleteTxForm({ tx, onClose }: Props) {
  ...
  const value = Math.abs(tx.amount / 100).toFixed(2)

  const mutation = useMutation({
    mutationFn: async () => deleteTransactionAction({ recurrence: tx.recurrence, id: tx.id }),
    mutationKey: ['delete-transaction', tx.id],
    onMutate() {
      onClose()
      client.setQueryData(['transactions'], (oldData: AbstractTransaction[]) => {
  ...
  return (
    <div className="flex flex-col gap-[20px] items-start">
      ...
      <DialogDescription>
        ...
        <span className={clsx('font-semibold', tx.amount < 0 ? 'text-red-600' : 'text-green-600')}>
          R$ {value}
        </span>
        .
      </DialogDescription>
